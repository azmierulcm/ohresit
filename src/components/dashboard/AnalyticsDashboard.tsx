...
import { useTransactions, computeStats } from '@/lib/hooks/useTransactions';
...
export default function AnalyticsDashboard() {
  ...
  const { totalSpend, dailyAvg, pctChange, trendData, categoryData, balance } =
    computeStats(transactions, timeframe);
  ...
  return (
    <div className="max-w-4xl mx-auto p-5 pb-24 space-y-8">
          ...
          {(['Week', 'Month'] as const).map((t) => (
            <button
              ...
              className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${
                timeframe === t ? 'bg-white text-zinc-900 shadow-sm' : 'text-zinc-500 hover:text-zinc-700'
              }`}
            >
              {t}
            </button>
        ...
        <Card className="p-6 rounded-[2.5rem] border-none bg-zinc-900 text-white shadow-xl shadow-zinc-200">
          <div className="flex justify-between items-start">
            <div>
              ...
              <h2 className="text-4xl font-bold mt-2">RM {totalSpend.toFixed(2)}</h2>
            </div>
          ...
          <div className="mt-6 flex items-center gap-2">
            <span className={`flex items-center text-sm font-bold ${pctUp ? 'text-rose-400' : 'text-emerald-400'}`}>
              {pctUp ? <ArrowUpRight className="h-4 w-4 mr-0.5" /> : <ArrowDownRight className="h-4 w-4 mr-0.5" />}
        ...
        <Card className="p-6 rounded-[2.5rem] border-zinc-100 shadow-sm bg-white">
          <div className="flex justify-between items-start">
            <div>
              ...
              <h2 className="text-4xl font-bold mt-2 text-zinc-900">RM {dailyAvg.toFixed(2)}</h2>
            </div>
          ...
          <div className="mt-6 flex items-center gap-2">
            <span className="text-zinc-500 text-xs">Balance: <strong className="text-zinc-900">RM {balance.toFixed(2)}</strong></span>
          </div>
        </Card>
      </div>
      ...
      <Card className="p-6 rounded-[2.5rem] border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] bg-white overflow-hidden">
        ...
        {trendData.length === 0 ? (
          <div className="h-[250px] flex items-center justify-center text-zinc-400 text-sm font-medium">
            No transactions this {timeframe.toLowerCase()} yet.
          </div>
        ) : (
          <div className="h-[250px] w-full -ml-6">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trendData}>
                ...
                <YAxis hide />
                <Tooltip contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', fontWeight: 'bold', fontSize: '12px' }} />
      ...
      {/* Category Split + Saving Opportunity */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-8 rounded-[2.5rem] border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] bg-white">
          ...
          {categoryData.length === 0 ? (
            <div className="h-[200px] flex items-center justify-center text-zinc-400 text-sm">No data yet.</div>
          ) : (
            <>
              <div className="h-[200px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <Tooltip formatter={(val) => `RM ${Number(val).toFixed(2)}`} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
              <div className="grid grid-cols-2 gap-3 mt-4">
                {categoryData.map((item) => (
                  <div key={item.name} className="flex items-center gap-2 px-3 py-2 bg-zinc-50 rounded-xl">
                    ...
                    <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-tight truncate">{item.name}</span>
                  </div>
                ))}
              </div>
            </>
        ...
        <Card className="p-8 rounded-[2.5rem] border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] bg-white flex flex-col justify-center items-center text-center">
          ...
          <h3 className="text-xl font-bold text-zinc-900 tracking-tight">Top Category</h3>
          {categoryData.length > 0 ? (
            <p className="text-zinc-500 text-sm mt-2 leading-relaxed">
              You spent <strong>RM {categoryData[0].value.toFixed(2)}</strong> on{' '}
              <strong>{categoryData[0].name}</strong> this {timeframe.toLowerCase()}.
            </p>
          ) : (
            <p className="text-zinc-500 text-sm mt-2">Add transactions to see your top spending category.</p>
          )}
          <button
            onClick={openEntryModal}
            className="mt-8 w-full py-4 bg-zinc-100 hover:bg-zinc-200 transition-colors rounded-2xl font-bold text-zinc-900 text-sm"
          >
            Add Transaction
          </button>
        </Card>
